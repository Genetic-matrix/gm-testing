...
2. Read `findings/D/summary.md` and `results.json` (tier tests) and `golden-master/captures/D/compare.md` (MaintVerify). If either is missing, the nightly run did not happen or failed: say so in the first line of the report.
